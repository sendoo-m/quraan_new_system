...
from accounts.models import User, SiteSettings, GeneralSupervisorHallAssignment
...
class UserCreateView(GeneralManagerRequiredMixin, View):
    ...
    def post(self, request):
        ...
        if form.is_valid():
            ...
            assigned_halls = request.POST.getlist('assigned_halls')
            if user.is_general_supervisor and assigned_halls:
                for hall_id in assigned_halls:
...
class UserUpdateView(GeneralManagerRequiredMixin, View):
    ...
    def get(self, request, pk):
        ...
        if user.is_general_supervisor:
            assigned_halls = user.hall_assignments.values_list('hall_id', flat=True)
    ...
    def post(self, request, pk):
        ...
        if form.is_valid():
            ...
            GeneralSupervisorHallAssignment.objects.filter(supervisor=user).delete()
            if user.is_general_supervisor:
                assigned_halls = request.POST.getlist('assigned_halls')
                for hall_id in assigned_halls:
                    GeneralSupervisorHallAssignment.objects.get_or_create(
...
class UserResetPasswordView(GeneralManagerRequiredMixin, View):
    def post(self, request, pk):
        ...
        user.save()
        messages.success(request, f'✅ تم إعادة تعيين كلمة مرور {user.get_full_name()}')
        return redirect('settings:users')
